                    ...
                    if save_obj:
                        if int(cls) ==0:
                            for k in range(len(det)):
                                ...
                                ###!!rescale image !!!
                                """ filename=p.name
                                filename = 'lp'+str(lp)+'.jpg'
                                filepath=os.path.join(r'test_images/results/', filename)
                                print(filepath) """
                                """ img = Image.open(filepath).convert("RGB")
                                print([ocr_printed_image(img,printed_processor, printed_model)]) """
                                lp += 1
...
def main(opt):
    ...
    print(vars(opt)['source'])
    #VARIABLES = vars(opt)
    event_handler=MonitorFolder()
    observer = Observer()
    observer.schedule(event_handler, path=vars(opt)['source'], recursive=True)
    ...
    try:
        while(True):
           time.sleep(1)
           print(VIDEO_FILES)
           while len(VIDEO_FILES) > 1:
               vars(opt)['source'] = VIDEO_FILES[0]
               print(vars(opt))
               run(**vars(opt))
               VIDEO_FILES.pop(0)
...
class MonitorFolder(FileSystemEventHandler):
    ...
    def on_modified(self, event):
        ...
        if(os.path.isfile(event.src_path)):
            if len(VIDEO_FILES) > 0 and  VIDEO_FILES[-1] != event.src_path:
